...
Future startnoti(id) async {
  ...
  final conn = await MySqlConnection.connect(ConnectionSettings(
      ...
      password: 'Gvg89rmaEG'));
  var phys = await conn
      .query('select * from physical_activity where p_id = ? limit 1', [uid]);
  if (phys.length >= 1) {
    for (var phy in phys) {
  ...
  var appo = await conn
      .query('select * from appointment where p_id = ? limit 1', [uid]);
  if (appo.length >= 1) {
    for (var app in appo) {
      app_date = app['a_date'];
      app_time = app['a_time'];
  ...
  var meds =
      await conn.query('select * from prescription where p_id = ?', [uid]);
  if (meds.length >= 1) {
    for (var med in meds) {
...
Future<void> notifcation({request, time}) async {
  ...
  if (request == 'test') {
    ...
    HomePageState.repeatdietNotification();
  }
}
...
class HomePageState extends State<HomePage> {
  ...
  static Future<void> repeatNotification() async {
    ...
    await flutterLocalNotificationsPlugin.periodicallyShow(
        0, 'maak', physical, physical_duration, platformChannelSpecifics,
        androidAllowWhileIdle: true);
  }
  ...
  static Future<void> repeatmedNotification() async {
    ...
    const NotificationDetails platformChannelSpecifics =
        NotificationDetails(android: androidPlatformChannelSpecifics);
    await flutterLocalNotificationsPlugin.periodicallyShow(0, 'maak medication',
        medic, RepeatInterval.daily, platformChannelSpecifics,
        androidAllowWhileIdle: true);
  }
  ...
  static Future<void> repeatdietNotification() async {
    ...
    await flutterLocalNotificationsPlugin.periodicallyShow(
        0,
        'maak diet',
        'حافظ على صحتك بمحافظتك على اكلك',
