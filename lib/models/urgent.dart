...
Future<dynamic> Urgent(phrase, id) async {
  //if critical is returned from dialogflow
  User_data user = new User_data(
      "`fname`,`relative_name`,`relative_phone`,`physician_id`",
      'patients',
      id);
  ...
  for (var res in respond) {
    ...
    if (physician_id != null) {
      User_data doctor =
          new User_data("`fname`,`email`", 'users', physician_id);
      ...
      for (var dres in result) {
        ...
        Response response = await post(localUrl, body: {
